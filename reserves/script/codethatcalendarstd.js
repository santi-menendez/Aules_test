...
function UA(){
	...
	this.opera5=n.indexOf("opera 5")>0||n.indexOf("opera/5")>=0;
	this.opera6=n.indexOf("opera 6")>0||n.indexOf("opera/6")>=0;
	this.opera7=n.indexOf("opera 7")>0||n.indexOf("opera/7")>=0;
	this.oldOpera=this.opera5||this.opera6;
	this.ie3down=n.indexOf('msie')!=-1&&v<4&&!this.opera;
	this.ie4up=!this.ie3down&&document.all&&document.all.item&&!this.opera;this.ie5up=this.DOM&&this.ie4up;
	this.ie6=this.ie5up&&n.indexOf('msie 6.')>0;
	this.ie=this.ie4up||this.ie5up;
	this.nn4=ns&&!this.DOM&&!this.opera;
	this.moz=this.nn6up=this.DOM&&ns&&!this.opera;
...
function CodeThatSetMY(m,y,f){
	...
	if(win&&win.codethatcalendar){
		win.codethatcalendar.date.setYear(y);
		win.codethatcalendar.date.setMonth(m);
	}
	location.reload();
};
	...
	if(d.getElementById&&Def(d.getElementById(f))){
		...
		if(d.forms.item)
			for (var i=0;d.forms.item(i)!=null;++i){
				if(d.forms.item(i).namedItem&&d.forms.item(i).namedItem(f)) return d.forms.item(i).namedItem(f);
				if(d.forms.item(i).elements&&d.forms.item(i).elements[f]) return d.forms.item(i).elements[f];
			}
			for (var form in d.forms)
				if(d.forms[form].elements&&d.forms[form].elements[f]) return d.forms[form].elements[f];
	}
	return null;
};
...

{
	...
	CTc.getCss=function(key,d){ return ""; };
	CTc.hide=function(){
	...
	CTc.create=function(d,ctl){
		...
		if(this.ifr&&def.headerstyle.type!="comboboxes"){
			acts='onMouseOver="window.opener.codethatcalendar.hideifr=false;"';
			acts+=" onMouseOut=\"window.opener.codethatcalendar.hideifr=true; setTimeout('window.opener.codethatcalendar.hide();',1000);\""
		}
	...
	CTc.createTime=function(d){
		d.write("<tr><td colspan=7 align=center>"+(ua.nn4?"<form>":"")+"<input "+(ua.nn4?"name":"id")+"='time' type=textarea value='"+CodeThatAlign(this.date.getHours())+":"+CodeThatAlign(this.date.getMinutes())+":"+CodeThatAlign(this.date.getSeconds())+"'>"+(ua.nn4?"</form>":"")+"</td></tr>");
	};
	...
	CTc.createButtons=function(d,i){
		...
		for(var p in dh)--col;
		if(dh.imgprevy&&!i)d.write("<td width=14% align=left><a href='javascript:CodeThatSetMY("+this.date.getMonth()+","+this.date.getFullYear()+",3)'><IMG border=0 src='"+dh.imgprevy+"'</IMG></A>");
		if(dh.imgprevm&&!i)d.write("<td width=14% align=left><a href='javascript:CodeThatSetMY("+this.date.getMonth()+","+this.date.getFullYear()+",1)'><IMG border=0 src='"+dh.imgprevm+"'</IMG></A>");
		d.write("<td colspan="+col+" align=center "+this.getCss("css",dh)+">"+this.def.monthnames[this.date.getMonth()]+"&nbsp;"+this.date.getFullYear()+"</td>");
		if(dh.imgnextm&&!i)d.write("<td width=14% align=right><a href='javascript:CodeThatSetMY("+this.date.getMonth()+","+this.date.getFullYear()+",0)'><IMG border=0 src='"+dh.imgnextm+"'</IMG></A></td>");
		if(dh.imgnexty&&!i)d.write("<td width=14% align=right><a href='javascript:CodeThatSetMY("+this.date.getMonth()+","+this.date.getFullYear()+",2)'><IMG border=0 src='"+dh.imgnexty+"'</IMG></A></td>");
	};
	...
	CTc.parseValue=function(s,d){
		...
		if(f.indexOf("yyyy")!=-1){
			...
			if(!isNaN(t)&&typeof(t)!='undefined')d.setYear(t);
		}
		if(f.indexOf("MM")!=-1){
	...
	CTc.popup2=function(ctl){
		window.codethatcalendar=this;
		this.ctl=ctl;
		var e=CodeThatFind(document,ctl);
		this.date=new Date();
		if(e.value!="")this.parseValue(e.value);
		var w=window.open("/intrafnb/aules/reserves/codethatcalendar.html","",'width='+this.def.windoww+',height='+this.def.windowh+',status=no,resizable=no,top=200,left=200,dependent=yes,alwaysRaised=yes');
		w.opener=window;
		w.focus();
	};
